...
const API_CONFIGS = {
  ...
  billionaires: {
    url: (year) => `https://forbes400.onrender.com/api/forbes400?limit=100&year=${year}`
  },
  movies: {
...
async function fetchAndCacheData(apiName, params = {}) {
  const baseDocRef = `${apiName}-${params.year || ''}`;
  ...
  try {
    ...
    if (metadataDoc.exists) {
      ...
      if (now - metadata.timestamp < CACHE_DURATION) {
        if (apiName === 'stocks') {
          const stocksDoc = await db.collection('top100')
        ...
        for (let i = 0; i < metadata.chunks; i++) {
          const chunkDoc = await db.collection('top100')
    ...
    const url = typeof config.url === 'function' ? config.url(params.year) : config.url;
    ...
    if (apiName === 'stocks') {
      ...
      chunks.forEach((chunk, index) => {
        const chunkRef = db.collection('top100').doc(`${baseDocRef}-chunk-${index}`);
      ...
      if (data.timestamp && now - data.timestamp >= CACHE_DURATION) {
        ...
        if (doc.id.endsWith('-metadata')) {
          ...
          for (let i = 0; i < data.chunks; i++) {
            const chunkRef = db.collection('top100').doc(`${baseDocRef}-chunk-${i}`);
...
async function cleanupStocksChunks() {
  try {
    const snapshot = await db.collection('top100')
